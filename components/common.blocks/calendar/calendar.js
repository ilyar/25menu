...
provide(bemDom.declBlock(this.name, /** @lends calendar.prototype */{
    ...
    parseDate: function(val) {
        ...
        var iso = moment(val, ['DD-MM-YYYY', 'YYYY-MM-DD', 'DD MMMM YYYY'], 'en');
    ...
    _changeMonthOnClick: function(e) {
        var elem = e.bemTarget;
        if (!elem.hasMod('disabled')) {
            this.switchMonth(elem.hasMod('direction', 'left') ? -1 : 1);
        }
    },
    ...
    onInit: function() {
        ...
        this._domEvents('arrow').on('click', ptp._changeMonthOnClick);
        this._domEvents('day').on('click', ptp._changeDayOnClick);
